...
function wp_template_pepper_one_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'wp-pepper-one' ),
		'<a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . $time_string . '</a>'
	);

	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'wp-pepper-one' ),
       ...
       // $GLOBALS['comment'] = $comment;
        ...
        if ( 'pingback' == $comment->comment_type || 'trackback' == $comment->comment_type ) : ?>
            ...
            <div class="comment-body">
                <?php _e( 'Pingback:', 'wp-pepper-one' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'wp-pepper-one' ), '<span class="edit-link">', '</span>' ); ?>
            </div>
                ...
                <div class="media-body">
                    <div class="media-body-wrap card">
                        ...
                        <div class="card-header">
                            <h5 class="mt-0"><?php printf( __( '%s <span class="says">says:</span>', 'wp-pepper-one' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?></h5>
                            <div class="comment-meta">
                                <a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
                                    <time datetime="<?php comment_time( 'c' ); ?>">
                                        <?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'wp-pepper-one' ), get_comment_date(), get_comment_time() ); ?>
                                    </time>
                                </a>
                                <?php edit_comment_link( __( '<span style="margin-left: 5px;" class="glyphicon glyphicon-edit"></span> Edit', 'wp-pepper-one' ), '<span class="edit-link">', '</span>' ); ?>
                            </div>
                        </div>

                        <?php if ( '0' == $comment->comment_approved ) : ?>
                            <p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'wp-pepper-one' ); ?></p>
                        <?php endif; ?>
