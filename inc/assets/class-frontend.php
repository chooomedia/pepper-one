    ...
    class WPSL_Frontend {
        ...
        public function create_opening_hours_tabel( $hours, $hide_closed ) {
            ...
            if ( $this->not_always_closed( $hours ) ) {
                $hour_table = '<table role="presentation" class="col-md-4 col-6 wpsl-opening-hours">';
                ...
                foreach ( $opening_days as $index => $day ) {
                    ...
                    if ( $hour_count > 0 ) {
                        ...
                        $hour_table .= '<td>' . __( 'Geschlossen', 'wpsl' ) . '</td>';
                    }
        ...
        public function show_store_address( $atts ) {
            ...
            $content .= '<div class="row flex-center-mobile mb-5"><a href="tel:+49' . $contact_details['phone'] . '" title="Fon" class="partner-buttons"><i class="fas fa-3x fa-phone"></i></i></a><a href="mailto:' . $contact_details['email'] . '" title="Email" class="partner-buttons"><i class="fas fa-3x fa-envelope"></i></a><a class="partner-buttons" title="Route" ><i class="fas fa-3x fa-map-marked-alt"></i></a></div>';
            $content .= '</div>';
        ...
        public function show_store_map( $atts ) {
            ...
            $output =   '<div id="wpsl-base-gmap_' . self::$map_count . '" class="col-md-6 wpsl-gmap-canvas"></div>' . "\r\n";
        ...
        public function check_map_shortcode_atts( $atts ) {
            ...
            if ( isset( $atts['height'] ) && is_numeric( $atts['height'] ) ) {
                $height = 'min-height:' . $atts['height'] . 'vh;';
            } else {
                $height = '';
            }
        ...
        public function get_custom_css() {
            ...
            if ( $wpsl_settings['template_id'] == 'below_map' && $wpsl_settings['listing_below_no_scroll'] ) {
                $css .= "\t" . "#wpsl-gmap {height:" . esc_attr( $wpsl_settings['height'] ) . "px;}" . "\r\n";
            ...
            $css .= "\t" . "#wpsl-search-input" . $cat_elem . " {width:" . esc_attr( $wpsl_settings['search_width'] ) . "%;}" . "\r\n";
            $css .= '</style>' . "\r\n";
